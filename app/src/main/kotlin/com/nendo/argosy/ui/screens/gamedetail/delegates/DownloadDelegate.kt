...
import com.nendo.argosy.ui.screens.gamedetail.modals.FilePickerRow
...
class DownloadDelegate @Inject constructor(
    ...
    private val downloadGameUseCase: com.nendo.argosy.domain.usecase.download.DownloadGameUseCase
) {
    ...
    /**
     * Builds picker rows for a game or null when there is nothing to choose
     * (single file, no versions): callers fall back to a plain download.
     */
    ...
    fun downloadWithSelection(
        ...
        scope.launch {
            val primaryRommId = gameRepository.getById(gameId)?.rommId
            var queued = 0
            if (primaryRommId == null || primaryRommId in selectedVersionIds || selectedVersionIds.isEmpty()) {
                ...
                when (val r = downloadGameUseCase(gameId, selectedFileIds = explicit)) {
                    ...
                    is DownloadResult.Error -> notificationManager.showError(r.message)
                    else -> {}
                }
            }
